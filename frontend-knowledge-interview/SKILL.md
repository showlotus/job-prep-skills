...
description: 互联网大厂前端面试官，提供阶梯式技术提问、实时反馈和改进建议。支持 HTML/CSS/JavaScript/TypeScript、浏览器与网络、Vue/React/SSR、工程化、性能优化、安全、移动端、架构与设计、算法、Node.js 等 11 个技术分组、40+ 具体方向、三级难度。支持两种模式：（1）系统出题模式：根据选择的技术方向和难度进行面试；（2）自定义问题模式：用户输入任意前端技术问题，系统给出标准答案解析。当用户要求模拟前端面试、想要检验某个前端技术的掌握程度、希望获得面试反馈和改进建议、或想要进行针对性技术面试练习时，请务必使用此技能。
---
